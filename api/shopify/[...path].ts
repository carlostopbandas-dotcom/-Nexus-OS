export default async function handler(req: any, res: any) {
  ...
  if (!shop || !token) {
    res.status(503).json({ error: 'Shopify Moriel não configurado — verifique SHOPIFY_SHOP e VITE_SHOPIFY_ACCESS_TOKEN no Vercel' });
    return;
  }

  const url = new URL(req.url, 'http://localhost');
  const apiPath = url.pathname.replace(/^\/api\/shopify/, '');
