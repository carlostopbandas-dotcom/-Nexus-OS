/**
 * Script de captura de screenshots baseline — Sprint 3
 * Story 1.3.1: Screenshot baseline das 9 páginas
 *
 * Uso:
 *   SCREENSHOT_EMAIL=seu@email.com SCREENSHOT_PASSWORD=senha node docs/baseline/capture-screenshots.js
 *
 * Ou sem credenciais (abre browser para login manual):
 *   node docs/baseline/capture-screenshots.js
 */
...
const OUTPUT_DIR = path.join(__dirname, 'sprint3');
...
async function captureScreenshots() {
  ...
  const needsLogin = currentUrl.includes('login') || currentUrl.includes('auth') || currentUrl !== BASE_URL + '/';
  ...
  console.log('\n📸 Capturando screenshots (1440x900)...\n');
  ...
  console.log(`\n✅ ${PAGES.length}/9 screenshots salvos em: docs/baseline/sprint3/`);
  console.log('\nArquivos:');
